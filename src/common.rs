...
impl ReminderManagerSender {
    ...
    pub async fn schedule(&self, reminder: Reminder) -> anyhow::Result<()>{
        self.0.send(ReminderManagerMessage::Schedule(reminder)).await?;
        Ok(())
    }
    
    pub async fn notify_error(&self, error: anyhow::Error, reminder: Reminder) -> anyhow::Result<()>{
        self.0.send(ReminderManagerMessage::ScheduleError(error, reminder)).await?;

        Ok(())
    }
    pub async fn notify_completed(&self, reminder: Reminder) -> anyhow::Result<()> {
        self.0.send(ReminderManagerMessage::ScheduleFinished(reminder)).await?;
