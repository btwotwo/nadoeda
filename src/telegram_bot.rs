...
impl TelegramInteractionInterface {
    pub async fn start() {
        ...
        let message_handler = Update::filter_message()
            ...
            .branch(case![CreateReminderDialogueState::ReceiveText].endpoint(Self::receive_reminder_text))
            .branch(dptree::endpoint(Self::invalid_state));

        let schema  = dialogue::enter::<Update, InMemStorage<CreateReminderDialogueState>, CreateReminderDialogueState, _>()
            .branch(message_handler);

        Dispatcher::builder(bot, schema)
            .dependencies(dptree::deps![InMemStorage::<CreateReminderDialogueState>::new()])
    ...
    async fn create_reminder_start(
        ...
        bot.send_message(msg.chat.id, "Creating a new reminder! Please input reminder text").await?;
        dialogue.update(CreateReminderDialogueState::ReceiveText).await?;
    ...
    async fn receive_reminder_text(
        ...
        match msg.text() {
            Some(text) => {
                bot.send_message(msg.chat.id, format!("Great! Your reminder will say \"{}\"", text)).await?;
                dialogue.update(CreateReminderDialogueState::ReceiveFiringTime { text: text.to_string() }).await?;
            },
            None => {
                bot.send_message(msg.chat.id, "Please send me reminder text.").await?;
            }
        }
    ...
    async fn invalid_state(bot: Bot, dialogue: ReminderCreationDialogue, msg: Message ) -> HandlerResult {
        bot.send_message(msg.chat.id, "Unable to handle the message.").await?;
        Ok(())
    }
}
